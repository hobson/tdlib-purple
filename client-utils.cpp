...
#define C(type) case td::td_api::type::ID: return #type;
    ...
    C(setDefaultMessageAutoDeleteTime)
...
void notifySendFailed(const td::td_api::updateMessageSendFailed &sendFailed, TdAccountData &account)
{
    if (sendFailed.message_) {
        ...
        if (chat) {
            ...
            if (sendFailed.error_ != nullptr) {
        if (auto err = dynamic_cast<const td::td_api::error*>(sendFailed.error_.get())) {
            errorMessage = formatMessage(errorCodeMessage(), {std::to_string(err->code_),
                                                     err->message_});
        }
    }
