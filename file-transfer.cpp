                                         ...
                                         TdTransceiver::ResponseCb sendMessageResponse)
{
    ...
    if (file.remote_) {
        if (file.remote_->is_uploading_active_) {
            ...
            size_t bytesSent = std::max(static_cast<td::td_api::int53>(0), file.remote_->uploaded_size_);
...
static void updateDownloadProgress(const td::td_api::file &file, PurpleXfer *xfer, TdAccountData &account)
{
    ...
    int32_t  downloadedSize = std::max((long int)0, file.local_ ? file.local_->downloaded_size_ : 0);
